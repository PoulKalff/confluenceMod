...
class HandleDB:
	...
	def __init__(self, db_type='sqlite'):
		...
		if db_type == 'sqlite':
			...
			self.conn = psycopg2.connect(user="postgres", host="127.0.0.1", port="5432", database='drtv')
		else:
	...
	def createTables(self):
		...
		self.cursor.execute("CREATE TABLE episodes(Title VARCHAR, PrimaryImageUri VARCHAR, Slug VARCHAR, SortDateTime VARCHAR, Uri VARCHAR, SeriesSlug VARCHAR, SeriesTitle VARCHAR, Description VARCHAR, Duration INTEGER)")
		self.cursor.execute("INSERT INTO lastUpdate(update_date) VALUES (date('now'))")
		self.conn.commit()
